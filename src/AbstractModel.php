...
abstract class AbstractModel extends Prefab
{
    ...
    public function generatePK()
    {
        ...
        $last = (int) $this->select(sprintf('right(%s, %s) as last',
                $pk, $format['serial']))
            ->order($pk.' desc')
            ->get(1)
            ->last;

        return $format['prefix'].str_pad($last+1,
            $format['serial'], '0', STR_PAD_LEFT);
    }
    ...
    **/
    public function copyfrom($key,$func=NULL) {
        ...
        foreach ($var as $key=>$val)
            !isset($this->schema['fields'][$key]) || $this->{$key} = $val;
        return $this;
    }
    ...
    **/
    ...
    {
        $params = array();
        $columns = explode('_', Instance::snakecase(
            str_replace('_', '-', lcfirst($method))));
